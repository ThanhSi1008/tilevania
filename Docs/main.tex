...
\begin{titlepage}
\begin{center}
{\Large \bf BỘ CÔNG THƯƠNG}\\ [0.5cm]
{\Large \bf TRƯỜNG ĐẠI HỌC CÔNG NGHIỆP TP.HCM}\\ [0.5cm]
{\large \bf KHOA CÔNG NGHỆ THÔNG TIN}\\[1.5cm]
\includegraphics[width=0.6\textwidth]{logo}\\ [1cm]
{\Large \bf BÁO CÁO DỰ ÁN}\\ [1cm]
{\Large \bf Unity Game + Server MongoDB Atlas}\\[1cm]
{\Large \bf MÔN: Công nghệ mới trong phát triển ứng dụng CNTT}\\[1cm]
\vspace{0.5cm}
{\LARGE \bf Giảng viên hướng dẫn: Ths.Nguyễn Ngọc Lễ}\\
\vspace{0.5cm}
{\LARGE \bf Nhóm: 23}\\
\vspace{0.5cm}
{\LARGE \bf Lớp: DHKHMT18A}\\
...
%------------------------------------------------------
% Danh sách thành viên và phân công công việc (gộp)
%------------------------------------------------------
\section*{Danh sách thành viên và phân công}
\addcontentsline{toc}{section}{Danh sách thành viên và phân công}
\begin{table}[H]
    \centering
    \begin{tabularx}{0.98\textwidth}{|c|c|c|X|}
        \hline
        \textbf{STT} & \textbf{MSSV} & \textbf{Họ và tên} & \textbf{Nhiệm vụ chi tiết} \\
        \hline
        1 & 22634631 & Phạm Đức Tài & 
        Thiết kế và lập trình client Unity: xây dựng gameplay platformer (di chuyển, nhảy, dash, bắn, va chạm), tối ưu animation và vật lý nhân vật; hiện thực HUD (Lives/Score/Level) gắn với GameSession; thiết kế UI cho AuthScene, MainMenu, Settings và tích hợp luồng gọi API (đăng nhập/đăng ký, start/update/end session, level progress) vào game. \\
        \hline
        2 & 22639301 & Lai Thanh Sĩ & 
        Phát triển backend Node.js/Express: thiết kế và hiện thực các API auth, gameProfile, sessions, levelProgress, levels; xây dựng schema và chỉ mục cho MongoDB Atlas (users, gameProfiles, gameSessions, levels, levelProgress, leaderboard, achievements); viết script seed dữ liệu demo; cấu hình và triển khai server trên Render (biến môi trường, kết nối Atlas, kiểm tra healthcheck và log). \\
        \hline
        3 & 22639861 & Hoàng Phi Hùng & 
        Thực hiện kiểm thử end-to-end giữa client và server (auth, lưu tiến trình, leaderboard/achievements, quay về MainMenu và chơi lại), xây dựng kịch bản test chi tiết; thu thập và phân tích log lỗi UI/UX và API; chuẩn hoá giao diện (layout, font, thông báo lỗi); biên soạn và hoàn thiện báo cáo (kiến trúc, ERD, API, UI/Gameplay, Atlas/Render) và hỗ trợ cấu hình CI/CD, theo dõi chất lượng hệ thống. \\
        \hline
    \end{tabularx}
\end{table}
...
\end{itemize}
\subsection{Sơ đồ luồng đăng nhập / phiên chơi (mô tả)}
\textbf{Luồng đăng nhập/đăng ký}:
\begin{itemize}[leftmargin=1.5em]
    \item Người chơi nhập thông tin → AuthScene gọi \texttt{/api/auth/login} hoặc \texttt{/api/auth/register}.
    \item Server xác thực, trả JWT + user, cập nhật \texttt{lastLoginAt}.
    \item Client lưu token, bật nhạc nền, tải profile (currentLives, currentLevel, totalScore), chuyển MainMenu.
\end{itemize}
\textbf{Luồng phiên chơi (start → sync → end)}:
\begin{itemize}[leftmargin=1.5em]
    \item \textit{Start}: Chọn level → \texttt{POST /api/sessions} (userId, levelId) \textrightarrow\ sessionId; tải/khởi tạo LevelProgress.
    \item \textit{Sync trong phiên}: định kỳ \texttt{PUT /api/sessions/:sessionId} gửi score, coins, enemiesDefeated, deathCount, livesRemaining.
    \item \textit{Complete level}: \texttt{POST /api/levelProgress/:userId/:levelId/complete} cập nhật bestScore/bestTime/playCount/isCompleted.
    \item \textit{End}: \texttt{POST /api/sessions/:sessionId/end} với trạng thái (COMPLETED/FAILED/ABANDONED) và duration; nếu mất kết nối có thể bỏ qua end.
\end{itemize}
...
\begin{itemize}[leftmargin=1.5em]
    \item \textbf{GET /api/levels}: trả danh sách level (phục vụ client mapping sceneName $\leftrightarrow$ levelId).
...
\begin{itemize}[leftmargin=1.5em]
    ...
    \end{itemize}
\subsection{Hình minh hoạ giao diện}
\begin{figure}[H]
    \centering
...
\begin{figure}[H]
    ...
    \caption{Màn hình Register}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{mainMenu.png}
    \caption{Main Menu}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{achivement.png}
    \caption{Màn hình Achievements}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{leaderboard.png}
    \caption{Màn hình Leaderboard}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{setting.png}
    \caption{Màn hình Settings âm thanh}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{level-1.png}
    \caption{Gameplay Level 1}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{level-2.png}
    \caption{Gameplay Level 2}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{level-3.png}
    \caption{Gameplay Level 3}
\end{figure}
...
\subsection{CI/CD}
\begin{itemize}[leftmargin=1.5em]
    \item Server: pipeline build/test (Jest), lint/validate; deploy Node/Express (commonjs) lên Render (web service). Có script seed (utils/seedDB) và leaderboard (utils/calculateLeaderboard).
    \item Biến môi trường Render: \texttt{MONGODB\_URI}, \texttt{DB\_NAME}, \texttt{JWT\_SECRET}, \texttt{PORT} (Render tự cấp); base URL API công khai để Unity gọi.
...
\begin{itemize}[leftmargin=1.5em]
    \item Biến môi trường: \texttt{MONGODB\_URI} (SRV), \texttt{DB\_NAME}, user Atlas tối thiểu quyền; IP whitelist (thêm outbound IP của Render).
